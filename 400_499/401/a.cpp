...
int main(){
    ...
    rep(i,0,n){
        if(t[i]=='?'){
            ll maru = 0;
            if(i+1<=n-1 and t[i+1] =='o') maru++;
            if(i-1 >= 0 and t[i-1] == 'o') maru++;  
            if(maru == 0) pos.insert(i);
        }
    }
    ...
    left_k = right_k = k;
    rep(i,0,n){
        if(left_t[i] == '?' and left_k!=0){
            ll maru = 0;
            if (i + 1 <= n - 1 and t[i + 1] == 'o') maru++;
            if (i - 1 >= 0 and t[i - 1] == 'o') maru++;
            if (maru == 0) {
                left_t[i] = 'o';
                if (i + 1 <= n - 1) left_t[i + 1] = '.';
                if (i - 1 >= 0) left_t[i-1] = '.';
                left_k--;
            }
        }
    ...
    rrep(i,n-1,0){
        if (right_t[i] == '?' and right_k != 0)
        {
            ll maru = 0;
            if (i + 1 <= n - 1 and t[i + 1] == 'o')
                maru++;
            if (i - 1 >= 0 and t[i - 1] == 'o')
                maru++;
            if (maru == 0)
            {
                right_t[i] = 'o';
    ...
    cerr << left_t << ", " << right_t << endl;
    cerr << left_k << ", " << right_k << endl;
    rep(i,0,n){
        if(left_t[i] == right_t[i] and left_t[i] == 'o') {
            t[i] = 'o';
        }
        else if(left_t[i] == right_t[i] and left_t[i] == '.') t[i] = '.';
    }
    cout << t << endl;
    // cerr << t << ", " << k << ", " << size(pos) << endl;
