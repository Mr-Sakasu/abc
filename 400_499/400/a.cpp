...
int main(){
    ...
    P t, g;
    cin >> t.first >> t.second >> g.first >> g.second;
    t.first--, t.second--;
    g.first--, g.second--;
    queue<P> q; q.push(t);
    map<P, bool> vis; vis[t] = true;
    while(not q.empty()){
        ...
        rep(i,0,4){
            ...
            P v = P{ny, nx};
            if(vis[v]) continue;
            if(not (0 <= ny and ny < h and 0 <= nx and nx < w and s[ny][nx] == '.')) continue;
            vis[v] = true;
            q.push(v);
        }
    }
    using State = tuple<ll, P>; //(回数, y, x)
    map<P, ll> cnt;
    priority_queue<State, vector<State>, greater<State>> pq;
    pq.emplace(cnt[g] = 0, g);
    while(not pq.empty()){
        ...
        // if (curCost > cnt[cv]) continue;
        auto [y, x] = cv;
        cerr << "(" << y << ", " << x << ") : " << curCost  << endl;
        rep(i,0,4){
            ll ny = y;
            ll nx = x;
            rep(j,0,3) {
