...
int main() {
    ...
    string a; cin >> a;
    ll m = size(a); // ~1e6
    vvvl dp(n+1, vvl(m, vl(2, 0)));
    // dp[i][j][k] := i層目のj番目の群の多数決でk=0or1にするのに最小な反転数
    vvl base(n + 1, vl(m, 0));
    rep(j, 0, m){
        ll bit = a[j] - '0';
        base[0][j] = bit;
    ...
    ll curCnt = m; 
    rep(i,1,n+1){
        ll groups = curCnt / 3; // 層iのグループ数
        rep(j,0,groups){
            ll idx0 = 3*j, idx1 = 3*j+1, idx2 = 3*j+2;
            ll sum = base[i - 1][idx0] + base[i - 1][idx1] + base[i - 1][idx2];
            ll curOrig = (sum >= 2 ? 1 : 0);
            base[i][j] = curOrig;
            ll flip0 = min(dp[i-1][idx0][0], dp[i-1][idx0][1]);
            ll flip1 = min(dp[i-1][idx1][0], dp[i-1][idx1][1]);
            ll flip2 = min(dp[i-1][idx2][0], dp[i-1][idx2][1]);    
            rep(k,0,2){
                ll cost1 = dp[i-1][idx0][k] + dp[i-1][idx1][k] + flip2;
                ll cost2 = dp[i-1][idx0][k] + dp[i-1][idx2][k] + flip1;
                ll cost3 = dp[i-1][idx1][k] + dp[i-1][idx2][k] + flip0;
                dp[i][j][k] = min({cost1, cost2, cost3});
            }
        }
        curCnt = groups;
    }

    cout << dp[n][0][1-base[n][0]] << endl;
