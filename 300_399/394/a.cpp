...
int main() {
    ...
    rep(i, 0, n) rep(j, 0, n) {
        ...
        if(c != '-') {
            ...
            in[j][c].emplace_back(i);
            out[i][c].emplace_back(j);
        }
    }
    while(!q.empty()) {
        auto [u, v] = q.front(); q.pop(); //i -> jは回文
        ll cnt = dp[u][v];
        for(char c = 'a'; c <= 'z'; ++c) { //両端にcを追加
            for(auto x : in[u][c]) {
                for(auto y : out[v][c]) {
                    // cerr << "c: " << c << ", " << x << "=>" << u << ", " << u << "=>" << y <<  endl;
                    // cerr << dp[x][y] << ", " << cnt << ", " << cnt + 2 << endl;
                    if(chmin(dp[x][y], cnt+2)){
                        q.push({x, y});
                    }
                }
            }
